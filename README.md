...
| `deploy/` | Production deploy guide (Cloud Run + Firebase Auth/Firestore). See [`deploy/README.md`](./deploy/README.md). |
...
Deploy so teammates and customers can open your **Cloud Run web** URL (see `deploy/README.md`):

1. **API** → Google Cloud Run (Gemini + Jira): `.\scripts\deploy-api-cloudrun.ps1`
2. **Web** → Cloud Run: `.\scripts\deploy-web-cloudrun.ps1` · **Firestore rules**: `.\scripts\deploy-firestore-rules.ps1`
