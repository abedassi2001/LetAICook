...
export function ProjectsList() {
  ...
  const hasJira = Boolean(jiraAuth);
  const loading = authLoading || (hasJira && jiraLoading);

  // One fetch per visit / refresh, or when Jira auth changes — not on every Firestore profile tick.
  useEffect(() => {
    if (authLoading || !user?.uid || !jiraAuth) return;
    ...
    void (async () => {
      ...
      setError(null);
      try {
        const list = await listJiraProjects(jiraAuth, manualCreds);
  ...
  }, [authLoading, user?.uid, jiraAuth, jiraDomain, jiraEmail, jiraApiToken, jiraDefaultProject, manualCreds]);
      ...
      {!hasJira ? (
        <div className="rounded-xl border border-app-border bg-app-elevated/80 p-5 ring-1 ring-white/[0.04]">
          ...
          <p className="mt-2 text-sm text-app-muted">
            Connect with Jira OAuth or API token in Settings, then return here to open a project
            and manage tasks.
          </p>
          <Link
            href="/settings"
      ...
      {hasJira ? (
        <>
          <p className="text-sm text-app-muted">
            Select a Jira project to view and manage tasks.
          </p>
